...
void* mmap( void* addr, size_t length, int prot, int flags, int fd, off_t offset )
{
    ...
    case PROT_READ:
    {
        hnd = CreateFileMapping( HANDLE( _get_osfhandle( fd ) ), nullptr, PAGE_READONLY, 0, 0, nullptr );
        if( hnd )
        {
    ...
    case PROT_WRITE:
    {
        hnd = CreateFileMapping( HANDLE( _get_osfhandle( fd ) ), nullptr, PAGE_READWRITE, 0, 0, nullptr );
        if( hnd )
        {
